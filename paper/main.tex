...
G_n=\sum_{k=0}^n4^k\binom{2n-2k}{n-k}^{2}\binom{2k}{k}.
\end{equation}
In Conjecture 2.7 of \cite{Sun2020}, he predicted the following all-depth
congruence.  If $p\equiv1\pmod4$ is prime, write
$p=x^2+4y^2$.  Then, for every positive odd integer $m$ and every $r\ge2$,
\begin{equation}\label{eq:Sun}
G_{(mp^r-1)/2}\equiv(4x^2-2p)G_{(mp^{r-1}-1)/2}
-p^2G_{(mp^{r-2}-1)/2}\pmod {p^r}.
\end{equation}

Our proof identifies the formal logarithm attached to \eqref{eq:Gdef} with
the Eichler integral of the weight-three CM form $\eta(4z)^6$.  Thus the
coefficient $4x^2-2p$ in \eqref{eq:Sun} is not accidental: it is the
$p$th Hecke eigenvalue of that form.  The remaining step is a strict
coordinate transfer for one-dimensional formal groups.
...
OpenAI Codex was used as the principal mathematical research tool in this
work.  All proof strategies and mathematical arguments establishing the
main result were generated by Codex.  It was also used for symbolic and
numerical checks, literature searches, and manuscript preparation.  The
human project director selected the problem and supervised the project.
Before journal submission, the named author will verify every argument,
computation, and citation and will assume full responsibility for the
contents.  The AI system is not an author.
...
included with the public proof-candidate packet.
